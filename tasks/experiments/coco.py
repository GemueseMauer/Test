...
def coco_annotations_to_tensors(coco_annotations,
    ...
    for ann_idx, ann in enumerate(annotations):
        #print(ann_idx)
        #print("ann:", annotations["keypoints"])
        kps = annotations['keypoints']
...
def coco_annotations_to_mask_bbox(coco_annotations, image_shape):
    ...
    #print(coco_annotations["num_keypoints"])
    for ann in coco_annotations:
        # print("ann:", ann)
        # print("coco_annotations[ann]:", coco_annotations[ann])
        # print("coco_annotations[ann]['num_keypoints']:", coco_annotations[ann]['num_keypoints'])
        if 'num_keypoints' not in coco_annotations or coco_annotations['num_keypoints'] == 0:
            bbox = coco_annotations['bbox']
            x0 = round(bbox[0])
            y0 = round(bbox[1])
            x1 = round(x0 + bbox[2])
            y1 = round(y0 + bbox[3])
...
class CocoDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for img in data['images']:
            img_map[data['images'][img]['id']] = data['images'][img]

        samples = {}
        for ann in data['annotations']:

            # filter by category
            if data['annotations'][ann]['category_id'] != cat_id:
                continue

            # filter by crowd
            if not use_crowd and data['annotations'][ann]['iscrowd']:
                continue

            img_id = data['annotations'][ann]['image_id']
            img = img_map[img_id]
            height = img['height']
            width = img['width']
            area = data['annotations'][ann]['area']

            # # filter by object area ### no area of forklift
            ...
            if img_id not in samples:
                sample = {}
                sample['img'] = img
                sample['anns'] = data["annotations"][ann]
